...
import os, sys; sys.path.append(os.path.dirname(bpy.data.filepath)); import texture_painter_cycles

## To reload after a code changes
import importlib; importlib.reload(texture_painter_cycles); texture_painter_cycles.go()
"""
...
def swap_texture_cycles(target_obj, image_filename, index):
    ...
    target_obj.material_slots[0].material = new_material
    new_img = bpy.data.images.load(image_filename)
    target_obj.material_slots[0].material.node_tree.nodes['Image Texture'].image = new_img
...
def swap_text(target_obj, backer, index):
    ...
    swap_texture_cycles(target_obj, filename, index)

def go():
    print("Texture Painter starting up.")
    cycles_render_select()
    print("Cycles Render Selected")
